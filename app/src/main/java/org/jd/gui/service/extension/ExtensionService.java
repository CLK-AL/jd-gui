...
public class ExtensionService {
			...
			if (path.startsWith("file:")) {
				...
				if (bangIndex > 0) {
					...
					try {
						...
						System.err.println("Failed to validate extension URL: " + url);
						assert ExceptionUtil.printStackTrace(e);
						return false;
					}
				}
			}
			System.err.println("Rejecting invalid jar URL for extension: " + url);
			return false;
		}

		if (!"file".equals(protocol)) {
			System.err.println("Rejecting non-file URL for extension: " + url);
			return false;
		}
	...
	private boolean validatePath(Path path, URL url) {
		...
		if (extensionsDir == null) {
			System.err.println("Extensions directory not set, rejecting: " + url);
			return false;
		}
		...
		if (!path.normalize().startsWith(extensionsDir.normalize())) {
			System.err.println("Rejecting extension outside extensions directory: " + path);
			return false;
		}
		...
		if (path.toFile().isFile() && !path.toString().toLowerCase().endsWith(".jar")) {
			System.err.println("Rejecting non-JAR extension file: " + path);
			return false;
		}
	...
	private Path extractJarPath(URL url) {
		try {
			...
			System.err.println("Failed to extract JAR path from URL: " + url);
			assert ExceptionUtil.printStackTrace(e);
		}
		return null;
	}
	...
	private boolean verifyJarSignature(Path jarPath) {
		...
		try (JarFile jar = new JarFile(jarPath.toFile(), true)) {
			...
			System.err.println("JAR signature verification failed: " + jarPath);
			assert ExceptionUtil.printStackTrace(e);
			return false;
		} catch (IOException e) {
			System.err.println("Error verifying JAR: " + jarPath);
			assert ExceptionUtil.printStackTrace(e);
			return false;
		}
	}
	...
	private void closeExtensionClassLoader() {
		if (extensionClassLoader != null) {
			try {
				...
				System.err.println("Failed to close extension class loader");
				assert ExceptionUtil.printStackTrace(e);
			}
		}
	}
