									      ...
									      if (nestedClassNode.type != ClassNode.CLASS_MEMBER) {
										      ...
										      for (int i = 0;
											      ...
											      if (param.type == Exprent.EXPRENT_VAR && mask.get(i) != null) {
												      ...
												      // Note: Variable finality flags are not reliably computed at this
												      // processing stage. The VarProcessor computes finality during a
												      // later phase (VarDefinitionHelper), so checking VAR_NON_FINAL here
												      // would incorrectly reject variables that are actually effectively
												      // final. We create the VarFieldPair unconditionally and let the
												      // finality be determined later when the variable is actually used.
												      // This ensures that captured variables in nested classes are
												      // correctly linked to their enclosing scope.
					                ...
					                public Exprent processExprent(Exprent exprent) {
						                ...
						                if (child.type == ClassNode.CLASS_ANONYMOUS
							                ...
							                if (invokeExpr.getFunctype() == InvocationExprent.TYP_INIT) {
								                ...
								                // Create a deep copy to preserve the original parameter expressions
								                // with their current variable names before any subsequent processing
								                // might modify them. This ensures that when the anonymous class is
								                // rendered, the super constructor parameters retain their meaningful
								                // names from the enclosing scope.
								                child.superInvocation = new InvocationExprent(invokeExpr);
								                return null;
							                }
						                }
		...
		for (ClassNode child : node.nested) {
			if (child.type == ClassNode.CLASS_LAMBDA) {
				...
				if (child.type == ClassNode.CLASS_LOCAL && child.enclosingMethod != null) {
					// First try to get the enclosing method from the current node
					MethodWrapper enclosingMethodWrapper = node.getWrapper()
					                                           .getMethods()
					                                           .getWithKey(child.enclosingMethod);

					// If not found in the current node, search in parent and other
					// enclosing classes. This handles cases like switch-on-enum where
					// the synthetic switch map class might reference an enclosing method
					// that exists in a different class in the enclosure hierarchy.
					if (enclosingMethodWrapper == null) {
						enclosingMethodWrapper = findEnclosingMethod(child);
					}
					...
					if (enclosingMethodWrapper != null) {
						...
						// The enclosing method could not be found in any enclosing class.
						// This can occur for synthetic classes generated by the compiler
						// (e.g., for switch-on-enum) that don't have a proper enclosing
						// context. Skip setting the local class definition in this case
						// as the class will be handled appropriately during rendering.
						DecompilerContext.getLogger()
						                 .writeMessage("Could not find enclosing method for local class "
						                               + child.classStruct.qualifiedName
						                               + " - skipping local class definition placement",
