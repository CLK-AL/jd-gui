...
package al.clk.gui.server.auth
...
import al.clk.gui.server.config.KeycloakConfig
import java.net.URL
...
data class KeycloakPrincipal(
     ...
     */
    fun isAdmin(): Boolean = hasRole("admin") || hasRole("gui-admin")
}
...
fun ApplicationCall.keycloakPrincipal(): KeycloakPrincipal? {
    ...
    return try {
        ...
        KeycloakPrincipal.fromJWT(jwt, "gui-server")
    } catch (e: Exception) {
        null
    }
}
