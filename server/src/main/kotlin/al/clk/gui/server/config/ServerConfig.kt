...
package al.clk.gui.server.config
    ...
    companion object {
        fun load(config: ApplicationConfig): KeycloakConfig {
            ...
            return KeycloakConfig(
                realm = keycloak.propertyOrNull("realm")?.getString() ?: "gui",
                authServerUrl = keycloak.propertyOrNull("auth-server-url")?.getString()
                    ?: System.getenv("KEYCLOAK_URL") ?: "http://localhost:8180",
                clientId = keycloak.propertyOrNull("client-id")?.getString() ?: "gui-server",
                ...
                jwksUrl = keycloak.propertyOrNull("jwks-url")?.getString()
                    ?: "${System.getenv("KEYCLOAK_URL") ?: "http://localhost:8180"}/realms/gui/protocol/openid-connect/certs",
                issuer = keycloak.propertyOrNull("issuer")?.getString()
                    ?: "${System.getenv("KEYCLOAK_URL") ?: "http://localhost:8180"}/realms/gui",
    ...
    companion object {
        fun load(config: ApplicationConfig): OpenfireConfig {
            ...
            return OpenfireConfig(
                ...
                resourceName = openfire.propertyOrNull("resource-name")?.getString() ?: "gui-server",
    ...
    companion object {
        fun load(config: ApplicationConfig): MatrixConfig {
            ...
            return MatrixConfig(
                ...
                orgRoomPrefix = matrix.propertyOrNull("org-room-prefix")?.getString() ?: "gui-org",
