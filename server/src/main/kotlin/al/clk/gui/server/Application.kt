...
import al.clk.gui.server.auth.configureKeycloakAuth
import al.clk.gui.server.bedework.BedeworkService
import al.clk.gui.server.config.ServerConfig
import al.clk.gui.server.di.MimeTypeRegistry
import al.clk.gui.server.di.allServerModules
import al.clk.gui.server.matrix.MatrixSocialService
import al.clk.gui.server.webdav.configureWebDav
import al.clk.gui.server.sync.configureSyncRoutes
import al.clk.gui.server.sync.VCardSyncManager
import al.clk.gui.server.xmpp.OpenfireOrgManager
import al.clk.gui.server.puml.configurePlantUmlRoutes
import al.clk.gui.server.svg.configureSvgRoutes
import al.clk.gui.server.handlers.HandlerRegistry
import al.clk.gui.server.handlers.configureHandlerRoutes
...
fun Application.module() {
    ...
    routing {
            ...
            call.respondJson(mapOf(
                ...
                "services" to mapOf(
                    "openfire" to mapOf(
                        ...
                        "status" to serviceStatus[al.clk.gui.server.sync.SyncService.OPENFIRE]?.isOnline
                    ),
                    "matrix" to mapOf(
                        "enabled" to config.matrix.enabled,
                        "status" to serviceStatus[al.clk.gui.server.sync.SyncService.MATRIX]?.isOnline
                    ),
                    "bedework" to mapOf(
                        "enabled" to config.bedework.enabled,
                        "status" to serviceStatus[al.clk.gui.server.sync.SyncService.BEDEWORK]?.isOnline
                    )
                ),
                "mimeTypes" to mapOf(
        ...
        get("/api/mime") {
            ...
            val types = if (category != null) {
                try {
                    val cat = al.clk.gui.server.di.MimeCategory.valueOf(category.uppercase())
    ...
    logger.info { "gui Server started on ${config.host}:${config.port}" }
}
